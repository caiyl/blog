                ...
                if (typeof toolbar == "string") {
                    ...
                    for (var i = 0; i < toolbar.length; i++) {
                        ...
                        if (t == "-" ) {
                            ...
                            if(t.type && "searchBox" == t.type){
                                var text = t.text ? t.text : "";
                                var html = "<div class='searchBoxDiv'>"
                                    + "<input type='text' style='border:none;' class='searchBoxInput' id='"+t.id+"' placeholder='"+text+"'/>"
                                    + "<input type='button' style='width:28px;' class='searchBoxButton' value title='过滤/查询'>"
                                    + "</div>";
                                var searchBox = $(html).appendTo(BtnGroup);
                                searchBox[0].childNodes[1].onclick = eval(t.handler || function () { });
                                searchBox.css("float" , "left");
                            }else{
                                //var tool = $("<a href=\"javascript:void(0)\" class='fm-button ui-state-default ui-corner-all ui-reset fm-button-icon-left " + p.iconCls + "'>" + p.text + "</a>").appendTo($(grid.uDiv));
                                var btnCls = t.btnCls?t.btnCls:'btn btn-info btn-small'
                                var btnEle = "<button class='"+btnCls+"' ";
                                if(tId != "") {
                                    btnEle += "id='"+tId+"'><i class='"+t.iconCls+"'></i> "+ t.text + "</button >";
                                }else {
                                    btnEle +="><i class='"+t.iconCls+"'></i> "+ t.text + "</button >";
                                }
                                var tool = $(btnEle).appendTo(BtnGroup);
                                //var tool = $("<button  class='"+btnCls+"'><i class='"+t.iconCls+"'></i> "+ t.text + "</button >").appendTo(BtnGroup);
                                tool[0].onclick = eval(t.handler || function () { });
                                tool.css("float" , "left");
