...
public class XmlUtil {
    public static void main(String[] args) throws DocumentException, IOException {
        String textFromFile = FileUtil.fileToString(new File("F:\\sunline\\jinhua\\jhwdservice\\testdata\\流程进度查询报文.xml"),ConfUtil.getValue("message.chartset"));
        Map<String, Object> map = xml2map(textFromFile.substring(6), true);
//        Document doc = map2xml(map, "root");
        ...
        if (list.size() > 0) {
            for (int i = 0; i < list.size(); i++) {
                ...
                if (iter.elements().size() > 0) {
                    Map m = xml2map(iter);
                    listElementToMap(map, iter, mapList, m);
                } else {
     ...
     * @param m
     */
    private static void listElementToMap(Map map, Element iter, List mapList, Map m) {
        if (map.get(iter.getName()) != null) {
            Object obj = map.get(iter.getName());
            if (!(obj instanceof List)) {
                mapList = new ArrayList();
